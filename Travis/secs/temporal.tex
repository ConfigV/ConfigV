...
Our solution is to buid a logical formula representing the entire history of examples, and use a SMT solver to find a classification model.
...
The summary must be \textit{sufficently detailed}, that is it must contain every piece of information that might lead to a build error.
...
Since we now consider both positive and negative examples, we will denote the build status of $P_t$ with $S(P_t)$.
...
With this notation, we can formally express the requirement that the program summary is sufficently detailed.
\begin{align}
...
We then can combine all these formulas with conjunctions and ask a SMT solver for a model satifsying the formula.
