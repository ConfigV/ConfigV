...
but all of the files in the sample need to be of the same type (such as  MySQL or httpd configuration files). From that sample
...
way of deriving  
a specification for configuration files. The learning process works in two phases. In the first phase \app analyzes the given dataset and generates a 
well-structured and probabilisticly-typed 
intermediate representation. In the second phase \app derives rules and constraints by analyzing
...
First, for variables of one types we learn different rules than for variables of different type. For example for integer variables $x$ and $Y$ we can learn that $X \leq Y$, while 
