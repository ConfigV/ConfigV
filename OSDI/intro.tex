...
Second, without templates, learning rules and constraints is 
another challenge.
Rather than relying on pre-defined templates or 
manually correcting training set,
we introduce and perform probabilistic learning approaches 
on probabilistically typed representations
generated in the transformation phase.
The learning process first analyzes the occurrences of each pair
of entries in the training set, 
then computes the likelihood that each pair can constitute a rule, 
and finally derive many rules.


Compared with previous efforts,
\app does not only rely on users' templates, it learns the rules about  
