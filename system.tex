...
Before describe how our system for learning rules is working, we first outline a translator which converts configuration files into an intermediary representation. Typing is based on already given set of basic types. However, typing is also a system module than can be easily extended to support
more types. In that case the user will need to provide rules for type inference and probability 

\subsection{From Untyped Expressions to Quantum Types}

Typing the untyped expressions appearing in configuration files 
is the first step in parsing and translating the data. 
